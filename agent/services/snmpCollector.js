...
const ENT_PHYSICAL_SN = '1.3.6.1.2.1.47.1.1.1.1.11';
...
async function collect(ip) {
  ...
  try {
    const [sysInfo, serial] = await Promise.all([
      ...
      (async () => { try { let v = await getOID(s, SNMP_OIDS.printer.general.serialNumber); if (v) return v.toString().trim(); for (const o of Object.values(SNMP_OIDS.vendor)) { if (o.serialNumber) { try { v = await getOID(s, o.serialNumber); if (v?.toString().trim()) return v.toString().trim(); } catch {} } if (o.serialNumber2) { try { v = await getOID(s, o.serialNumber2); if (v?.toString().trim()) return v.toString().trim(); } catch {} } if (o.serialNumber3) { try { v = await getOID(s, o.serialNumber3); if (v?.toString().trim()) return v.toString().trim(); } catch {} } if (o.serialNumber4) { try { v = await getOID(s, o.serialNumber4); if (v?.toString().trim()) return v.toString().trim(); } catch {} } } try { v = await getOID(s, ENT_PHYSICAL_SN); if (v?.toString().trim()) return v.toString().trim(); } catch {} return null; } catch { return null; } })(),
    ]);
